...
class GravityOffence(Base):
    ...
    gravity_name: Mapped[str] = mapped_column(
...
class CrimeHead(Base):
    ...
    act_sections: Mapped[list[CrimeHeadActSection]] = relationship(
        back_populates="crime_head"
    )
...
class CrimeSubHead(Base):
    ...
    sub_head_name: Mapped[str] = mapped_column(
...
class Act(Base):
    ...
    crime_head_sections: Mapped[list[CrimeHeadActSection]] = (
        relationship(back_populates="act")
    )
...
class CrimeHeadActSection(Base):
    ...
    act: Mapped[Act] = relationship(
        back_populates="crime_head_sections"
    )
...
class CasteMaster(Base):
    ...
    caste_id: Mapped[int] = mapped_column(
        Integer, primary_key=True, autoincrement=True
    )
    caste_name: Mapped[str] = mapped_column(
        String(100), nullable=False
    )
...
class ReligionMaster(Base):
    ...
    religion_name: Mapped[str] = mapped_column(
        String(100), nullable=False
    )
