...
def seed_reference_data(session: Session) -> dict:
    ...
    for d_name in DESIGNATIONS:
        dg = Designation(designation_name=d_name)
        session.add(dg)
        session.flush()
    ...
    for name in CRIME_HEADS:
        ...
        for _ in range(random.randint(1, 3)):
            sh = CrimeSubHead(
                sub_head_name=f"{name} — {fake.word().title()}",
    ...
    section_map: dict[tuple[str, str], tuple[int, int]] = {}
    ...
    for _ in range(count):
        ...
        if random.random() < 0.6:
            sampled = random.sample(
                ...
                k=min(
                    len(accused_names),
                    random.randint(1, num_accused),
                ),
            )
            for aname in sampled:
                has_arrest = random.random() < 0.7
                ar = ArrestSurrender(
                    case_id=case.case_id,
                    accused_name=aname,
...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        print(
            f"Reference data: {len(ref['district_ids'])} "
            f"districts, {len(ref['unit_ids'])} units, "
            f"{len(ref['emp_ids'])} employees"
        )
